...
class _UserHomePageState extends State<UserHomePage> with TickerProviderStateMixin {
  ...
  final List<Map<String, dynamic>> moods = [
    ...
    {'name': 'Lonely', 'icon': Icons.sentiment_very_dissatisfied, 'color': Colors.purpleAccent},
  ...
  void initState() {
    ...
    _mistController = AnimationController(vsync: this, duration: const Duration(seconds: 10))..repeat(reverse: true);
    // fetch motivation when the user home is opened
    try {
      widget.controller.fetchMotivation();
    } catch (_) {}
  }
  ...
  void _saveEntry() {
    final moodName = (selectedMoodIndex >= 0) ? moods[selectedMoodIndex]['name'] as String : 'Unspecified';
    ...
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Entri tersimpan ke riwayat kalender')));
    setState(() {});
  }
            ...
            onPressed: () {
              Navigator.of(context).push(MaterialPageRoute(builder: (_) => Scaffold(
                ...
                body: Padding(
                  ...
                  child: Column(
                    children: [
                      CircleAvatar(radius: 46, child: Text(widget.user.name.isNotEmpty ? widget.user.name[0].toUpperCase() : 'U', style: const TextStyle(fontSize: 36))),
          ...
          AnimatedBuilder(
            ...
            builder: (context, _) {
              ...
              return Container(
                ...
                child: Stack(
                  children: [
                    Positioned(top: 120 + move, left: 50 + move, child: _buildMistBlob(200, Colors.tealAccent.shade100)),
                    Positioned(bottom: 100 - move, right: 40 - move, child: _buildMistBlob(250, Colors.pink.shade100)),
                  ],
                ),
              );
            },
          ...
          Padding(
            ...
            child: LayoutBuilder(
              builder: (context, constraints) {
                final columnCount = constraints.maxWidth < 600 ? 2 : constraints.maxWidth < 900 ? 3 : 4;
                final entries = controller.entriesFor(widget.user.username);

                return Column(
                  children: [
                    // Motivation / Affirmation banner
                    Obx(() {
                      final text = widget.controller.motivation.value;
                      ...
                      return AnimatedContainer(
                        ...
                        child: Row(children: [const Icon(Icons.lightbulb, color: Colors.amber), const SizedBox(width: 8), Expanded(child: Text(text, style: const TextStyle(fontStyle: FontStyle.italic)))]),
                      );
                    }),
                    AnimatedContainer(
                        ...
                        children: [
                          Row(
                            children: [
                              ...
                              Text('Catat Mood & Stress', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: Colors.indigo.shade700)),
                              const Spacer(),
                              ElevatedButton.icon(
                                onPressed: _pickDate,
                            ...
                            child: GridView.builder(
                              ...
                              itemBuilder: (context, i) {
                                ...
                                return GestureDetector(
                                  onTap: () => setState(() => selectedMoodIndex = isActive ? -1 : i),
                                  child: AnimatedContainer(
                                    ...
                                    decoration: BoxDecoration(
                                      ...
                                      gradient: isActive
                                          ? LinearGradient(colors: [m['color'] as Color, (m['color'] as Color).withAlpha(180)])
                                          : LinearGradient(colors: [(m['color'] as Color).withAlpha(80), Colors.white]),
                                      boxShadow: [
                                        BoxShadow(color: (m['color'] as Color).withAlpha(isActive ? 120 : 40), blurRadius: isActive ? 20 : 6, offset: const Offset(0, 6)),
                                      ],
                                    ),
                                    child: Center(
                                      child: Column(
                                        ...
                                        children: [
                                          Icon(m['icon'] as IconData, size: isActive ? 36 : 28, color: isActive ? Colors.white : (m['color'] as Color)),
                                          const SizedBox(height: 6),
                                          Text(m['name'] as String, style: TextStyle(color: isActive ? Colors.white : Colors.black87, fontWeight: FontWeight.w600, fontSize: isActive ? 14 : 12)),
                                        ],
                                      ),
                                    ),
                                  ),
                          ...
                          Row(
                            children: [
                              ...
                              ElevatedButton.icon(
                                onPressed: () {
                                  Navigator.push(context, MaterialPageRoute(builder: (_) => DailyJournalPage(initialText: _journalCtrl.text)));
                                },
                              ...
                              TextButton(
                                onPressed: () {
                                  Navigator.of(context).push(MaterialPageRoute(builder: (_) => UserCalendarPage(username: widget.user.username, controller: controller)));
                                },
                    ...
                    Expanded(
                      child: AnimatedContainer(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text('Ringkasan Terbaru', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
                            const SizedBox(height: 8),
  ...
  Widget _buildLatestEntryPreview() {
    ...
    if (entries.isEmpty) {
      return Text('Belum ada entri. Silakan catat mood dan jurnal harian.', style: TextStyle(color: Colors.grey.shade700));
    }
    final last = entries.last;
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('${last.mood} • Stress ${last.stressLevel}/10', style: const TextStyle(fontWeight: FontWeight.w700)),
        const SizedBox(height: 6),
        Text(last.note.isEmpty ? '(tidak ada catatan)' : last.note),
        const SizedBox(height: 6),
        Text('Pada: ${last.timestamp.day}/${last.timestamp.month}/${last.timestamp.year} ${last.timestamp.hour}:${last.timestamp.minute.toString().padLeft(2, '0')}', style: const TextStyle(fontSize: 12, color: Colors.black54)),
      ],
    );
  }

  Widget _buildRecentList() {
    final entries = widget.controller.entriesFor(widget.user.username).reversed.toList();
    if (entries.isEmpty) {
      return Center(child: Text('Kosong - belum ada entri', style: TextStyle(color: Colors.grey.shade600)));
    }
  ...
  Widget _buildMistBlob(double size, Color color) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        gradient: RadialGradient(colors: [color.withAlpha(100), Colors.transparent]),
      ),
    );
  }
}
...
class UserCalendarPage extends StatelessWidget {
  ...
  final HomeController controller;
  const UserCalendarPage({super.key, required this.username, required this.controller});
          ...
          children: [
            ...
            Expanded(
              child: GridView.builder(
                ...
                gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 7, crossAxisSpacing: 6, mainAxisSpacing: 6, childAspectRatio: 1),
                itemBuilder: (context, i) {
                  final d = monthDays[i];
                    ...
                    child: Container(
                      ...
                      child: Center(
                        child: Column(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Text('${d.day}', style: TextStyle(color: has ? Colors.white : Colors.black87, fontWeight: FontWeight.bold)),
        ...
        child: Hero(
          ...
          child: Container(
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  const Text("✨ Write your thoughts here ✨", textAlign: TextAlign.center, style: TextStyle(fontSize: 18, color: Colors.white, fontStyle: FontStyle.italic)),
                  const SizedBox(height: 12),
                  Expanded(
                    child: TextField(
                      maxLines: null,
...
class DosenHomePage extends StatefulWidget {
  final String username;
  final HomeController controller;
  const DosenHomePage({super.key, required this.username, required this.controller});

  @override
  State<DosenHomePage> createState() => _DosenHomePageState();
}

class _DosenHomePageState extends State<DosenHomePage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // fetch motivation for dosen home
    try {
      widget.controller.fetchMotivation();
    } catch (_) {}
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.logout), onPressed: () => Navigator.of(context).popUntil((r) => r.isFirst)),
        ],
      ),
      body: Padding(
        padding: const EdgeInsets.all(12.0),
          ...
          children: [
            Obx(() {
              ...
              return Card(
                ...
                child: Padding(padding: const EdgeInsets.all(12), child: Row(children: [const Icon(Icons.lightbulb_outline, color: Colors.deepPurple), const SizedBox(width: 8), Expanded(child: Text(text, style: const TextStyle(fontStyle: FontStyle.italic)))])),
              );
            }),
            SizeTransition(
              ...
              child: Card(
                ...
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                child: ListTile(
                  leading: CircleAvatar(backgroundColor: Colors.deepPurple.shade100, child: const Icon(Icons.monitor_heart, color: Colors.deepPurple)),
            ...
            Expanded(
              child: users.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, i) {
                        final u = users[i];
                        final count = widget.controller.entriesFor(u.username).length;
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            trailing: IconButton(
                              ...
                              onPressed: () {
                                Navigator.of(context).push(MaterialPageRoute(builder: (_) => DosenViewUserProfileWithHistory(user: u, controller: widget.controller)));
                              },
                            ),
                          ),
                        );
...
class DosenViewUserProfileWithHistory extends StatelessWidget {
  final User user;
  final HomeController controller;
  const DosenViewUserProfileWithHistory({super.key, required this.user, required this.controller});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            CircleAvatar(radius: 46, child: Text(user.name.isNotEmpty ? user.name[0].toUpperCase() : 'U', style: const TextStyle(fontSize: 36))),
            const SizedBox(height: 12),
            ...
            Expanded(
              child: entries.isEmpty
                  ? Center(child: Text('Belum ada entri', style: TextStyle(color: Colors.grey.shade600)))
                  : ListView.separated(
