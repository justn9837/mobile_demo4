...
Future<void> main() async {
  ...
  await AuthService.initServices();
...
class _LoginPageState extends State<LoginPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _onLogin() async {
    ...
    final userRemote = await AuthService.login(u, p);
    if (userRemote == null) {
    ...
    final user = User(username: userRemote.username, name: userRemote.name, age: userRemote.age, major: userRemote.major, email: userRemote.email, password: p);
...
class _RegisterPageState extends State<RegisterPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _onRegister() async {
    ...
    final u = app_models.User(
      username: formLocal.username,
      name: formLocal.name,
      age: formLocal.age,
      major: formLocal.major,
      email: formLocal.email,
      password: formLocal.password,
    );

    final ok = await AuthService.register(u);
    if (!ok) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Username sudah ada')));
      return;
    }
    Navigator.pushAndRemoveUntil(context, PageRouteBuilder(pageBuilder: (_, anim, __) => FadeTransition(opacity: anim, child: UserHomePage(user: formLocal)), transitionDuration: const Duration(milliseconds: 420)), (r) => false);
  }
...
class _UserHomePageState extends State<UserHomePage> with TickerProviderStateMixin {
  ...
  void _saveEntry() {
    ...
    AuthService.saveEntry(app_models.JournalEntry(
  ...
  @override
  Widget build(BuildContext context) {
    final entries = AuthService.entriesFor(widget.user.username);
...
class UserCalendarPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final remoteList = AuthService.entriesFor(username);
    final list = remoteList.map((e) => JournalEntry(username: e.username, mood: e.mood, stressLevel: e.stressLevel, note: e.note, timestamp: e.timestamp)).toList();
    for (var e in list) {
...
class _DailyJournalPageState extends State<DailyJournalPage> with TickerProviderStateMixin {
  ...
  Future<void> _autoSaveDraft() async {
    ...
    try {
      await AuthService.saveEntry(app_models.JournalEntry(
            ...
            Expanded(
              child: users.isEmpty
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (c, i) {
                        final u = users[i];
                        final count = AuthService.entriesFor(u.username).length;
                        final localUser = User(username: u.username, name: u.name, age: u.age, major: u.major, email: u.email, password: '');
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            trailing: IconButton(
                              ...
                              onPressed: () => Navigator.push(context, PageRouteBuilder(pageBuilder: (_, anim, __) => FadeTransition(opacity: anim, child: DosenViewUserProfileWithHistory(user: localUser)), transitionDuration: const Duration(milliseconds: 380))),
                            ),
                          ),
                        );
                      }),
...
class DosenViewUserProfileWithHistory extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final entries = AuthService.entriesFor(user.username);
    return Scaffold(
